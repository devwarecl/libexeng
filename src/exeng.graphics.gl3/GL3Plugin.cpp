...
namespace exeng { namespace graphics { namespace gl3 {
    ...
    void GL3Plugin::initialize(Root *root) {
        if (this->root != nullptr) {
            throw std::runtime_error("GL3Plugin::initialize: Can't initialize the plugin if already been initialized.");
        } 
    ...
    void GL3Plugin::terminate() {
        if (this->root == nullptr) {
            throw std::runtime_error("GL3Plugin::terminate: Can't terminate the plugin if has not been initialized.");
        } 
...
extern "C" { 
	EXENG_EXPORT exeng::system::Plugin* EXENG_CALLCONV ExengGetPluginObject() {
