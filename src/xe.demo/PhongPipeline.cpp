...
namespace xe {
    ...
    void PhongPipeline::render(xe::gfx::Mesh *mesh) {
        assert(mesh);
    
        graphicsDriver->render(mesh);
    }
    
    void PhongPipeline::setModel(const xe::Matrix4f &transformation) {
        programmableModule->setProgramMatrix("model", transformation);
    }
    
    const xe::gfx::VertexFormat* PhongPipeline::getVertexFormat() const {
        return &vertexFormat;
    }
