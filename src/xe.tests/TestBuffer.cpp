...
// class MeshSubsetHeap : public xe::gfx
...
BOOST_AUTO_TEST_CASE(TestBuffer) 
{
	...
	BOOST_CHECK_EQUAL(std::memcmp(values1, buffer1.getPointer(), sizeof(values1)), 0);	
	BOOST_CHECK_EQUAL(std::memcmp(values2, buffer2.getPointer(), sizeof(values2)), 0);	
	...
	BOOST_CHECK_NE(std::memcmp(values3, buffer3.getPointer(), buffer3.getSize()), 0);
	BOOST_CHECK_NE(std::memcmp(values4, buffer4.getPointer(), buffer4.getSize()), 0);

	BOOST_CHECK_EQUAL(std::memcmp(&values3[3], buffer3.getPointer(), buffer3.getSize()), 0);
	BOOST_CHECK_EQUAL(std::memcmp(&values4[1], buffer4.getPointer(), buffer4.getSize()), 0);
}
