...
namespace exeng { namespace raytracer { namespace renderers {
    ...
    struct HardwareRenderer::Private {
    public:
    ...
    public:
        static cl::Buffer createBuffer(cl::Context &context, const MaterialLibrary *library) {
            cl::Buffer materialBuffer;
	    ...
	    void computeSynthesisBuffer(const SceneNodeData *nodeData) {
			// std::ofstream fs;
    ...
    HardwareRenderer::HardwareRenderer(Texture *renderTarget, const AssetLibrary *assets, const MaterialLibrary *materialLibrary) {
        this->impl = std::make_unique<HardwareRenderer::Private>();
